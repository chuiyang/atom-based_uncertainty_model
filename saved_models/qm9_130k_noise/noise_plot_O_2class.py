...
def barChart(smiles, uncertainty, x_label, std, unc_tag, model_i=0):
    ...
    for smile, unc in zip(smiles, uncertainty):
        ...
        numF = 1 if numF > 1 else numF
        O_dict[numF] = locateCount(O_dict[numF], unc)

    cs = []

    for i in range(2):
        classSum = np.sum(O_dict[i])
        cs.append(int(classSum))
        print(f'{i}: {classSum}')
        for j in range(5):
            O_dict[i][j] = O_dict[i][j] / classSum


    # start to plot
    x = np.arange(len(x_ticks))  # the label locations
    width = 0.25  # the width of the bars
    fig, ax = plt.subplots(figsize=(6, 4))  # [6.4, 4.8]
    if unc_tag == 'ale':
        bar1 = ax.bar(x - width/2, y_0O, width, label=f'without Oxygen ({cs[0]})', color='#DACC96')
        bar2 = ax.bar(x + width/2, y_O, width, label=f'with      Oxygen ({cs[1]})', color='#9D5353')    
        # bar3 = ax.bar(x + width/2, y_2O, width, label='2O', color='#9D5353')
        # bar4 = ax.bar(x + 3*width/2, y_3Oup, width, label='3O_up', color='#632626')
    elif unc_tag == 'epi':
        bar1 = ax.bar(x - width/2, y_0O, width, label=f'without Oxygen ({cs[0]})', color='#E7E0C9')
        bar2 = ax.bar(x + width/2, y_O, width, label=f'with      Oxygen ({cs[1]})', color='#6B7AA1')    
    ...
    plt.savefig(f'./O_noise_plot_std{std}_{unc_tag}_1e_{model_i}_2class.png', dpi=650)





def uncertaintyCountNumN(filepath, std, unc_tag, model_i=0):
    ...
    barChart(smiles, unc, title, std, unc_tag, model_i)
...
if __name__ == '__main__':
    ...
    # for std in [0, 1, 3]:
        # uncertaintyCountNumN(f'./qm9_130k_pear_scale_2l_30e_lr_std{std}_post/fold_0/qm9_130k_noise_test_pear_scale_2l_30e_lr_std{std}_post.csv', std, 'epi')
    std = 0
    model_i = 5
    # filename = f'./qm9_130k_pear_scale_2l_30e_lr_std{std}_post/fold_0/qm9_130k_noise_test_pear_scale_2l_30e_lr_std{std}_post.csv'
    filename = f'./qm9_130k_pear_scale_2l_30e_lr_std{std}/each_pred/qm9_130k_noise_test_pear_scale_2l_30e_lr_std{std}_{model_i}.csv'
    ...
    uncertaintyCountNumN(filename, std, 'ale', model_i)
    # uncertaintyCountNumN(filename, std, 'epi')
