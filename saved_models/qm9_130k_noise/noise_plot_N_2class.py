...
def barChart(smiles, uncertainty, x_label, std, unc_tag, model=None):
    ...
    if unc_tag in ['ale', 'error']:
        bar1 = ax.bar(x - width/2, y_0N, width, label=f'without Nitrogen ({cs[0]})', color='#DACC96')
        bar2 = ax.bar(x + width/2, y_N, width, label=f'with      Nitrogen ({cs[1]})', color='#9D5353')    
        # bar3 = ax.bar(x + width/2, y_2N, width, label='2N', color='#9D5353')
        # bar4 = ax.bar(x + 3*width/2, y_3Nup, width, label='3N_up', color='#632626')
    elif unc_tag == 'epi':
        bar1 = ax.bar(x - width/2, y_0N, width, label=f'without Nitrogen ({cs[0]})', color='#E7E0C9')
        bar2 = ax.bar(x + width/2, y_N, width, label=f'with      Nitrogen ({cs[1]})', color='#6B7AA1')    
    ...
    plt.savefig(f'./noise_plot_std{std}_{unc_tag}_15e_2class.png', dpi=650)
...
if __name__ == '__main__':
    ...
    # uncertaintyCountNumN(f'./qm9_130k_pear_scale_2l_30e_lr_std{std}/fold_0/qm9_130k_noise_test_pear_scale_2l_30e_lr_std{std}.csv', std, 'error')
        # uncertaintyCountNumN(f'./qm9_130k_pear_scale_2l_30e_lr_std{std}/each_pred/qm9_130k_noise_test_pear_scale_2l_30e_lr_std{std}_{model}.csv', std, 'error', model)
    # std = 0
