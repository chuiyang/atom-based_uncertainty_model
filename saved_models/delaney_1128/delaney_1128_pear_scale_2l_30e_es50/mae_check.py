...
rmses = []
for i in range(30):
    file = pd.read_csv(f'./each_pred/delaney_1128_test_pear_scale_2l_30e_es50_{i}.csv').values
    true = file[:, 1]
    pred = file[:, 2]
...
file = pd.read_csv(f'./fold_0/delaney_1128_test_pear_scale_2l_30e_es50.csv').values
true = file[:, 1]
pred = file[:, 2]
mae = np.mean(abs(true-pred))
rmse = np.sqrt(np.mean(abs(true-pred)**2))
