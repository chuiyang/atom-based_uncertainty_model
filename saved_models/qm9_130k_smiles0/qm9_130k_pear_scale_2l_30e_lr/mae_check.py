...
rmses = []
for i in range(30):
    file = pd.read_csv(f'./each_pred/qm9_130k_smiles0_test_pear_scale_2l_30e_lr_{i}.csv').values
    true = file[:, 1]
    pred = file[:, 2]
...
file = pd.read_csv(f'./fold_0/qm9_130k_smiles0_test_pear_scale_2l_30e_lr.csv').values
true = file[:, 1]
pred = file[:, 2]
mae = np.mean(abs(true-pred))
rmse = np.sqrt(np.mean(abs(true-pred)**2))
...
plt.errorbar(0, np.mean(maes), yerr=np.std(maes), capsize=3, label='Single Model', marker='o')
