...
def autolabel(rects, ax, index):
    """Attach a text label above each bar in *rects*, displaying its height."""
    for i, rect in enumerate(rects):
        height = rect.get_height()
        if (index in [2, 3]) and (i==4):
            print(f'rect.get_x(), rect.get_width(): {rect.get_x(), rect.get_width()}')
            ax.annotate(f'{(height*100):.1f}',
                            xy=(rect.get_x() + rect.get_width() / 2-0.02, height-0.01),
                            xytext=(0, 3),  # 3 points vertical offset
                            textcoords="offset points",
                            ha='center', va='bottom', fontsize=7)
        elif (index == 4) and (i==4):
            ax.annotate(f'{(height*100):.1f}',
...
def barChart(smiles, uncertainty, x_label, unc_tag, model=None):
    ...
    plt.savefig(f'./noise_plot_{unc_tag}_15e_seed20_all_post_v2_barfloat.png', dpi=650)
...
if __name__ == '__main__':
    ...
    # for model in range(15):
        # uncertaintyCountNumN(f'./qm9_130k_pear_scale_2l_15e_withoutN_seed20_all/each_pred/qm9_130k_withoutN_test_pear_scale_2l_15e_withoutN_seed20_all_{model}.csv', 'ale', model)
    # filename = f'./qm9_130k_pear_scale_2l_15e_withoutN_seed20_smiles0/fold_0/qm9_130k_withoutN_test_pear_scale_2l_15e_withoutN_seed20_smiles0.csv'
    # filename = f'./qm9_130k_pear_scale_2l_15e_withoutN_seed20_all_post/fold_0/qm9_130k_withoutN_test_pear_scale_2l_15e_withoutN_seed20_all_post.csv'
    filename = f'./qm9_130k_pear_scale_2l_15e_withoutN_seed20_all_post/fold_0/qm9_130k_withoutN_test_pear_scale_2l_15e_withoutN_seed20_all_post.csv'
