...
from chemprop.train import active_learning_scf
...
if __name__ == '__main__':
    ...
    active_log = open(f'{args.save_dir}/active_log.csv', 'w')
    writer = csv.writer(active_log)
    writer.writerow([args.active_uncertainty,'','',''])
    active_log.close()

    for active_iter in range(0, 7):
        logger = create_logger(name=f'train_atl_{active_iter}', save_dir=args.save_dir, quiet=args.quiet, active_iter=active_iter)
        active_learning_scf(args, logger, active_iter=active_iter, logger_all=logger_all)
    
    # plot results
    active_log = pd.read_csv(f'{args.save_dir}/active_log.csv', header=None).values[1:, :]
    print(f'active log shape: {active_log.shape}')
