...
def draw_with_index(i, smiles_i, mol_unc_i, atomic_unc_i, unc_t, args):
    ...
    if unc_t == 'pred':
        ...
        d2d.DrawMolecule(mol_with_atom_index(molInd, atomic_unc))
        mix = False


    # elif mol_unc < 1.2:
    if mix: # only background
        ...
        # markdown draw mol in the below function (built-in function in rdkit)
    ...
    if mix:
        temp_path = os.path.join(args.unc_type_png_path, 'temp')
        os.mkdir(temp_path) if not os.path.isdir(temp_path) else None
        with open(os.path.join(temp_path, f'{i}_{unc_t}_bg.svg'), 'w') as f:
            f.write(png_data)
        # open(os.path.join(temp_path, f'{i}_{unc_t}_bg.svg'),'wb+').write(png_data) 
    else:
    ...
    if mix:
        ...
        # open(os.path.join(temp_path, f'{i}_{unc_t}_mol.svg'),'wb+').write(png_data2)
        with open(os.path.join(temp_path, f'{i}_{unc_t}_mol.svg'), 'w') as f:
            f.write(png_data2)

        # blend image
        # background = Image.open(os.path.join(num_path, f'{i}_{unc_t}.svg'))
        # overlay = Image.open(os.path.join(num_path, f'{i}_{unc_t}_mol.svg'))
        background = sg.fromfile(os.path.join(temp_path, f'{i}_{unc_t}_bg.svg'))
        overlay = sg.fromfile(os.path.join(temp_path, f'{i}_{unc_t}_mol.svg'))
        ...
        background.save(os.path.join(args.unc_type_png_path, f'{i}_{unc_t}.svg'))
...
def make_predictions_atomicUnc_multiMol(args: Namespace, smiles: List[str] = None, draw: bool = True) -> None:
    ...
    # print(f'Saving predictions to {args.png_path}')
