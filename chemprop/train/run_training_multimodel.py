...
from .evaluate_multimodel import evaluate_multimodel, evaluate_predictions_multimodel
...
from chemprop.utils import build_optimizer_multimodel, build_lr_scheduler, get_loss_func, get_metric_func, load_checkpoint,\
    makedirs, save_checkpoint, transfer_learning_release_varlayer, build_lr_scheduler_inverse
import pandas as pd


def run_training_multimodel(args: Namespace, logger: Logger = None) -> List[float]:
    """
    ...
    for model_idx in range(args.ensemble_size):
        # Tensorboard writer
        save_dir_i = os.path.join(args.save_dir, f'model_{model_idx}')
        makedirs(save_dir_i)
        # try:
        #     writer = SummaryWriter(log_dir=save_dir_i)
        # except:
        #     writer = SummaryWriter(logdir=save_dir_i)
        # Load/build model
        if args.checkpoint_paths is not None:
            ...
            if args.transfer_learning_freeze_GCNN:
                model, names_to_release = transfer_learning_release_varlayer(model, args.transfer_learning_freeze_GCNN, model_idx, logger=logger)  # transfer learning, freeze GCNN layer.
        else:
            raise ValueError('Only for transfer learning')
        ...
        save_checkpoint(os.path.join(save_dir_i, f'model.pt'), model, scaler, features_scaler, args)

        
        models_dict[f'model_{model_idx}'] = model


    # Optimizers
    optimizer = build_optimizer_multimodel(models_dict, args, names_to_release)
    ...
    for epoch in range(args.epochs):
        ...
        val_scores, val_rmses, val_maes = evaluate_multimodel(
            models_dict=models_dict,
            data=val_data,
        ...
        if args.minimize_score and avg_val_score < best_score or \
            ...
            model_rmse, model_mae = avg_val_rmse, avg_val_mae
            for model_idx, model_name in enumerate(models_dict.keys()):
                save_dir_j = os.path.join(args.save_dir, f'model_{model_idx}')
                save_checkpoint(os.path.join(save_dir_j, f'model.pt'), models_dict[model_name], scaler, features_scaler, args)
        else:
            early_stopping_step += 1
        
        # break if early stopping happens
        if early_stopping_step == early_stopping: 
            info(f'STOPPING CONDITION IS MET!! epoch:{epoch-early_stopping}')
            break
        
    info(f'val_heterosedastic:{best_score:.3f}, val_rmse: {model_rmse:.3f}, val_mae: {model_mae:.3f}')
    return best_score, model_rmse, model_mae
